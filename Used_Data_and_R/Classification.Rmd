...
full_cut <- full_data1 %>%
  ...
  left_join(regions_cut, by=c("statefips"="fips.x"))
```
```{r}
full_cut <- full_cut[,-1]
...
region_bag_OOB_output2 %>% 
  roc_curve(Region, Yes, event_level = "second") %>% autoplot()

region_bag_OOB_output2 %>% 
  roc_auc(Region, Yes, event_level = "second") #Area under Curve
```
...
heart_rf_OOB_output %>% 
  bag_metrics(truth = AHD, estimate = .pred_class)
```
...
heart_rf_OOB_output2 %>% 
  roc_curve(AHD, Yes, event_level = "second") %>% autoplot()

heart_rf_OOB_output2 %>% 
  roc_auc(AHD, Yes, event_level = "second") #Area under Curve
```
...
heart_rf_wf %>% #based on permutation
  ...
  fit(data = heart) %>% extract_fit_engine() %>% vip()
```
