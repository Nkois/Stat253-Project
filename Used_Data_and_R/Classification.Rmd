...
set.seed(253) # Pick your favorite number to fill in the parentheses
```

```{r}
policy_cut <- policy %>%
  select(statename, statefips, all_restrictions)%>%
  filter(statename != "")
...
#full_cut <- full_cut %>%
 # mutate(isSouth = relevel(0, ref='failure')) #set reference level

full_cut2$isSouth <- as.factor(full_cut2$isSouth)
full_cut2$all_restrictions <- as.factor(full_cut2$all_restrictions)

full_cut_sub3 <- full_cut2 %>%
  select(gps_away_from_home, case_rate, hospitalized_rate, spend_remoteservices,spend_hcs, emp_incbelowmed, all_restrictions)
...
# Recipe
logistic_rec <- recipe(all_restrictions ~ ., data = full_cut_sub3) %>%
...
logistic_output <- logistic_output %>%
  mutate(.pred_class = make_two_class_pred(.pred_0, levels(all_restrictions), threshold = .5)) #Try changing threshold (.5, 0, 1, .2, .8)

# Visualize Soft Predictions
logistic_output %>%
  ggplot(aes(x = all_restrictions, y = .pred_1)) +
  geom_boxplot() + 
...
# Confusion Matrix
logistic_output %>%
  conf_mat(truth = all_restrictions, estimate = .pred_class)
...
logistic_output %>% 
  log_metrics(estimate = .pred_class, truth = all_restrictions, event_level = "second") # set second level of outcome as "success"
```
...
logistic_roc <- logistic_output %>% 
    roc_curve(all_restrictions, .pred_1, event_level = "second") # set second level of outcome as "success"
...
Quinn Frankovsky - Reflection: 
I would assume that pillar 2 and pillar 4 are the hardest to get right for groups or companies that rely on data analytics, pillar 2 representing methods and pillar 4 representing engagement. I say pillar 2 because the methods or algorithms that we have learned about are greatly different from each other in terms of effectiveness, style, and other characteristics. In addition to their own quirks, each individual algorithm has their benefits and drawbacks depending on what is required. This can make selection of the algorithms quite tricky in some cases. In addition, the data that is used is critical to the outcome that is required. Inherent biases play into data collection, which severely alter the outcome of the algorithm. Lastly, pillar 4 is quite tricky as well. It is easier said than done, I imagine, to convince the public of your outcomes when they do not have the same analytical skill set. Making sure that your conclusions are crystal clear is very important to convincing the public of the outcome. 
