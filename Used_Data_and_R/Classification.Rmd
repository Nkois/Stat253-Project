...
full_cut$isSouth <- as.factor(full_cut$isSouth)

full_cut_sub3 <- full_cut %>%
  select(gps_away_from_home, case_rate, hospitalized_rate, spend_remoteservices,spend_hcs, emp_incbelowmed, isSouth)
...
# Recipe
logistic_rec <- recipe(isSouth ~ ., data = full_cut_sub3) %>%
...
logistic_output <- logistic_output %>%
  mutate(.pred_class = make_two_class_pred(.pred_0, levels(isSouth), threshold = .5)) #Try changing threshold (.5, 0, 1, .2, .8)

# Visualize Soft Predictions
logistic_output %>%
  ggplot(aes(x = isSouth, y = .pred_1)) +
  geom_boxplot() + 
...
# Confusion Matrix
logistic_output %>%
  conf_mat(truth = isSouth, estimate = .pred_class)
...
logistic_output %>% 
  log_metrics(estimate = .pred_class, truth = isSouth, event_level = "second") # set second level of outcome as "success"
```
...
logistic_roc <- logistic_output %>% 
    roc_curve(isSouth, .pred_1, event_level = "second") # set second level of outcome as "success"
