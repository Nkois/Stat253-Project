...
#for no-penalty lasso
#mnMod_cv_lasso <- fit_resamples(mn_lasso_wf_tune,
#  resamples = folded_mn,
 # metrics = metric_set(rmse, rsq, mae))
...
#Residual Plots
#OLS
mn_mod_output <- minnesota %>%
  bind_cols(predict(mnFullMod_cv, new_data = minnesota)) %>%
...
#LASSO
mn_mod_output_lasso <- minnesota %>%
  bind_cols(predict(final_fit, new_data = minnesota)) %>%
    mutate(resid = gps_away_from_home - .pred)
