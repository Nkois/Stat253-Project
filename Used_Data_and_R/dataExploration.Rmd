...
minnesota_cut <- minnesota %>%
  ...
  select(Date, gps_away_from_home, case_rate, hospitalized_rate, spend_remoteservices, spend_hcs, emp_incbelowmed, emp_incq1, emp_incq2, emp_incq3, emp_incq4)
...
gps <-ggplot(minnesota_cut, aes(y = gps_away_from_home, x = Date)) +
    geom_point() +
    geom_smooth(color = "blue", se = FALSE) +
    geom_hline(yintercept = 0, color = "red") + 
    theme_classic()

#vaccine <-ggplot(minnesota_cut, aes(y = fullvaccine_rate, x = Date)) +
 #   geom_point() +
 #   geom_smooth(color = "blue", se = FALSE) +
  #  geom_hline(yintercept = 0, color = "red") + 
 #   theme_classic()

case<-ggplot(minnesota_cut, aes(y = case_rate, x = Date)) +
    geom_point() +
    geom_smooth(color = "blue", se = FALSE) +
    geom_hline(yintercept = 0, color = "red") + 
    theme_classic()

hosp<-ggplot(minnesota_cut, aes(y = hospitalized_rate, x = Date)) +
    geom_point() +
    geom_smooth(color = "blue", se = FALSE) +
    geom_hline(yintercept = 0, color = "red") + 
    theme_classic()

remote<-ggplot(minnesota_cut, aes(y = spend_remoteservices, x = Date)) +
    geom_point() +
    geom_smooth(color = "blue", se = FALSE) +
    geom_hline(yintercept = 0, color = "red") + 
    theme_classic()
...
grid.arrange(gps, case, hosp, remote, hcs, ncol=2)
```


```{r employment_date_visualizations}
#Visualizing employment variables by date
emp1 <- ggplot(minnesota_cut, aes(y = emp_incq1, x = Date)) +
    geom_point() +
    geom_smooth(color = "blue", se = FALSE) +
    geom_hline(yintercept = 0, color = "red") + 
    theme_classic()+
...
emp2 <-ggplot(minnesota_cut, aes(y = emp_incq2, x = Date)) +
    geom_point() +
    geom_smooth(color = "blue", se = FALSE) +
    geom_hline(yintercept = 0, color = "red") + 
    theme_classic()+
...
emp3 <-ggplot(minnesota_cut, aes(y = emp_incq3, x = Date)) +
    geom_point() +
    geom_smooth(color = "blue", se = FALSE) +
    geom_hline(yintercept = 0, color = "red") + 
    theme_classic()+
...
emp4 <-ggplot(minnesota_cut, aes(y = emp_incq4, x = Date)) +
    geom_point() +
    geom_smooth(color = "blue", se = FALSE) +
    geom_hline(yintercept = 0, color = "red") + 
    theme_classic()+
...
grid.arrange(case_corr, hosp_corr, remote_corr, hsc_corr, ncol=2)
```
...
grid.arrange(emp1_corr, emp2_corr, emp3_corr, emp4_corr, empUnder_corr, ncol=2)
...
full_rec <- recipe(gps_away_from_home ~  case_rate + hospitalized_rate + emp_incq1 + emp_incq2 + emp_incq3 + emp_incq4 + spend_remoteservices+ spend_hcs + emp_incbelowmed, data=minnesota_cut) %>%
...
mn_mod <- mn_model_wf %>% fit(data=minnesota_cut)

```

```{r ols_output}
mn_mod_output_OLS <- mn_mod %>%
  predict(new_data=minnesota_cut) %>%
  bind_cols(minnesota_cut)%>%
...
full_lasso_rec <- recipe(gps_away_from_home ~  case_rate + hospitalized_rate  + emp_incq1 + emp_incq2 + emp_incq3 + emp_incq4 + spend_remoteservices+ spend_hcs + emp_incbelowmed, data=minnesota_cut) %>%
  step_normalize(all_numeric_predictors()) %>%
  step_dummy(all_nominal_predictors())%>%
  step_nzv(all_predictors())
...
final_fit <- fit(final_wf, data = minnesota_cut)
